...
class Translator():
    ...
    def __init__(self):
        ...
        self.OPERATION_URL = self.SERVER_URL + "operation"
    ...
    def sendOpPush(self, user, operation_list):
        payload = {"device":user, "push_list":operation_list}
        requests.post(url=self.OPERATION_URL, headers=self.Send_Header, data=json.dumps(payload))
